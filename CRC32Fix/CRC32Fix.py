#
# I Love Horses (NDS) CRC32 Patcher
...
for i in glob.iglob('./**/*.sav', recursive=True):
    if os.path.getsize(i) == 0x2000:	#size of the overall savefile
        with open(i, 'rb+') as f:
            data = f.read(0x934)		#reads the offset of the CRC-bytes
            if data[0x0:0x4] == b'\x00\x66\xff\x00':	#sanity-check for a specific string to be found in the save
                print('I Love Horses (NDS) CRC32 Patcher 1.0\nBy ihaveahax and ChampionLeake\n')   
